...
%		\textbf{Решение:} ...
		...
		\begin{enumerate}
			\item $n^i \leq n^d \leq n^k$ по условию $\Rightarrow$ $\sum\limits_{i=0}^{d} a_i n^i \leq \sum\limits_{i=0}^{d} a_i n^k$\\
			$\sum\limits_{i=0}^{d} a_i n^k = (\sum\limits_{i=0}^{d} a_i) n^k$\\
			$\sum\limits_{i=0}^{d} a_i$ можно обозначить за $C \Rightarrow C>0$, тк $a_i > 0$ по условию\\
			Следовательно, $p(n) = O(n^k)$
			\item Рассмотрим старший коэффициент многочлена $- a_d n^d$\\
			$n^d \geq n^k$, тк $k \leq d$ по условию $\Rightarrow p(n) \geq a_d n^d \geq a_d n^k$\\
			В качестве $C$ можно обозначить $a_d$ и тк $a_i > 0$ по условию $\Rightarrow C>0$\\
			Следовательно, $p(n) = \Omega(n^k)$
			\item Тк выполняются обе оценки, то $p(n) = \Theta(n^k)$ по определению\\
		\end{enumerate}
